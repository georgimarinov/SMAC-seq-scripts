...
def run():
    ...
    if len(sys.argv) < 8:
        print 'usage: python %s methylation_reads_all.tsv peak_list chrFieldID leftFieldID rightFieldID strandFieldID tabix_path outfile_prefix [-resize factor] [-subset N] [-minCov fraction] [-minPassingBases fraction] [-unstranded] [-minAbsLogLike float] [-scatterPlot colorscheme minScore maxScore color|none] [-window bp] [-readStrand +|-] [-printMatrix] [-deleteMatrix] [-binarize threshold]' % sys.argv[0]
...
#        print 'will only output regions with at least', MR, 'reads covering the region'
    ...
    for chr in chrList:
        ...
        for RL in posList:
            for (RR,RS) in RegionDict[chr][RL]:
                ...
                maxScoresLength = 0
                for (chr,readleft,readright,readstrand,read,cgs,loglike) in RegionReads:
                    scores = []
                    CGscoreDict = {}
                    CGs = cgs.split(',')
                    LLs = loglike.split(',')
    ...
    print len(Matrix), 'reads found'

    if doSubset:
        Matrix = random.sample(Matrix,Nsub)

    print len(Matrix), 'reads retained'
...
#        continue
#    if len(NewMatrix) < MR:
#        print len(NewMatrix), 'reads left for', chr, RL, RR, 'too few, skipping'
#        continue
